...
class KPOINTS():
    ...
    def __add__(self, other) :
        new = KPOINTS(reciprocal = self.reciprocal)
        if self.reciprocal == other.reciprocal :
            new.data = self.data.append(other.data).reset_index(drop = True)
            new.nkpt = self.nkpt + other.nkpt
            new.loc = new.data.loc
            new.u = self.u
            new.v = self.v
            new.w = self.w
            return new
    ...
    def get_knorm(self, kpt_line,u = None,v = None,w = None):
        ...
        for line in kpt_line:
            ...
            k_ref = kx+ky+kz
            #k_ref = np.dot(self.loc[kpt,['kx','ky','kz']],np.array([u,v,w]))
            ...
            for kpt in range(ini+p,end+f, incr):
                ...
                kz = self.loc[kpt]['kz']*w
                k = kx+ky+kz
                #k = np.dot(self.loc[kpt,['kx','ky','kz']],np.array([u,v,w]))
